...
```
 ...
 ```bash
dist
└── main.js  asset main.js 6.33 KiB [emitted] (name: main)
```

## webpack 添加配置 usedExports: true 打包
```bash
dist
└── main.js  asset main.js 5.1 KiB [emitted] (name: main)
```

## usedExports 
> 这个使用了之后 会提供未使用的代码的 标识，压缩工具的无用代码的清楚都会受这个选项的影响，可以去除未使用的导出内容
...
eval("/* unused harmony exports a, b, c */\nfunction a() { console.log(\"a\"); }\nfunction b() { console.log(\"b\"); }\nfunction c() { console.log(\"c\"); }\n\n\n//# sourceURL=webpack:///./abc.js?");
...
### 结果 
> library 虽然是导入了 abc, 但是这个 app 里面，并没有使用 abc 里面的方法，所以在使用了 usedExports： 配置 , library没有把 abc 打到一起，abc 也被打上了 **unused harmony exports** 标签。这个在压缩的时候会被删掉

> 而没有使用 usedExports 配置，library 把 abc 打到了一起
